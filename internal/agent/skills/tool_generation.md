...
## 5. 安全与限制 (强制要求)
- **禁止指令**: 禁止生成任何执行系统命令 (`os/exec`, `syscall`)、修改磁盘敏感位置或暴力数据库操作 (`DROP`, `TRUNCATE`) 的代码。
- **输入校验**: 必须对用户提供的输入参数进行基本的合法性校验（如非空校验、范围校验）。
- **硬编码限制**: 禁止包含任何硬编码的密钥、Token、密码或其他敏感凭证。
- **并发安全**: 若涉及共享资源，需考虑并发安全（如使用 `sync.Mutex`）。

## 6. 禁止事项
- 禁止使用 `init()` 函数。
- 禁止从外部文件读取不安全数据。
- 逻辑务必简洁，复杂逻辑可提供 mock 实现作为回退，降低生成阶段截断风险。
- 禁止在代码中直接使用 `panic()`，应通过 `error` 返回错误。
